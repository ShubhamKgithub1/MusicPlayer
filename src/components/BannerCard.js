...
const BannerCard = ({topTracks})=>{
    ...
    return (
        <div className="w-full h-[40%] flex relative rounded-3xl overflow-hidden ">
            <img src={track?.album?.cover_big} alt="not found" className="w-full relative object-cover"/>
            <div className="flex flex-col justify-end items-start absolute z-10 bottom-0 text-white p-6 bg-gradient-to-tr from-black to-transparent inset-0 gap-2">
                 <h1 className="text-4xl font-bold">{track?.album?.title}</h1>
                 <h3 className="text-xl font-medium">{track?.artist?.name}</h3>
                 <div className="flex gap-2">
                 <button className="border px-6 py-2 rounded-full hover:bg-gray-200 hover:text-black transition-all duration-300 active:scale-90" onClick={()=>handlePlay()}>Play</button>
