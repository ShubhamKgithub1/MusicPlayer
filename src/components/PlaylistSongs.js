...
const PlaylistSongs = ({song, playlistId, songId, userId, dispatch})=>{
    ...
    if(!song) return;
    return (
        <div className="flex gap-2 rounded-md items-center w-full bg-white/5 hover:shadow-[inset_0_0_8px_gray] dark:hover:shadow-black md:hover:shadow-none dark:hover:bg-white/10 p-2 md:py-2 md:px-4 snap-start relative group cursor-pointer animate-fade-in transition-all duration-200">
            ...
            <div className="truncate">
                ...
                <button className={`flex items-center ml-auto text-red-500 p-2 rounded-full hover:shadow-[inset_0_2px_4px_black] active:scale-[0.90] hover:bg-white/10 bg-transparent transition-all duration-200 dark:hover:bg-transparent`}
